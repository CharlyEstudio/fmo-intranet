...
// Config
import { URL_SERVICIO_GENERAL, URL_LOCAL, PUERTO_INTERNO, URL_PRUEBAS, PUERTO_SERVER, URL_PETICION, PUERTO_INTERNO_DOS } from '../../config/config';
...
export class GpsService {
  ...
  obtenerIMEIAsesorAll(desde: number = 0) {
    if (URL_SERVICIO_GENERAL === URL_PETICION) {
      ...
      this.url = URL_PRUEBAS + ':' + PUERTO_INTERNO_DOS + '/gps/desde?desde=' + desde;
    } else {
  ...
  actualizarUsusarioImei( usuario: Visor ) {
    if (URL_SERVICIO_GENERAL === URL_PETICION) {
      ...
      this.url = URL_PRUEBAS + ':' + PUERTO_INTERNO_DOS + '/gps/' + usuario._id;
    } else {
  ...
  borrarUsuarioImei( id: string ) {
    if (URL_SERVICIO_GENERAL === URL_PETICION) {
      ...
      this.url = URL_PRUEBAS + ':' + PUERTO_INTERNO_DOS + '/gps/' + id;
    } else {
  ...
  nuevoUsuarioIMEI(visor: Visor) {
    if (URL_SERVICIO_GENERAL === URL_PETICION) {
      ...
      this.url = URL_PRUEBAS + ':' + PUERTO_INTERNO_DOS + '/gps';
    } else {
